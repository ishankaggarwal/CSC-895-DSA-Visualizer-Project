...
import { CycleDetectionVisulizationInterface } from "@/app/interfaces/GraphSearchVisualizationInterface";
import { cycleDetection } from "@/app/visualization-algorithms/CycleDetection";

function sleep(ms: number) {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

const CycleDetectionInGraphVisualizer = () => {
  const { speedValue, isPlaying, setMarkers } = useContext(AppContext);
  ...
  const animationsRef = useRef<CycleDetectionVisulizationInterface[]>([]);
  ...
  const visualizeGraph = () => {
    ...
    animations = cycleDetection(graph, startNode);
    if (animationsRef.current.length > 0) {
      setAnimations([]);
      resetNodesAndPaths();
    } else {
...
export default CycleDetectionInGraphVisualizer;
