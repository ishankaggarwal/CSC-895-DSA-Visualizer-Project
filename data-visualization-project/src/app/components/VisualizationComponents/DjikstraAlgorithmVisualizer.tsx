...
const DjikstraAlgorithmVisualizer = () => {
  ...
  const [graph, setGraph] = useState<Map<string, [string, number][]>>(
    new Map<string, [string, number][]>()
  );
  const [startNode, setStartNode] = useState<string>("");
  const animationsRef = useRef<DjikstraAlgorithmVisualizationInterface[]>([]);
  const [animations, setAnimations] = useState<
    DjikstraAlgorithmVisualizationInterface[]
  >([]);
  const [queue, setQueue] = useState<number[]>([]);
  const [matrix, setMatrix] = useState<Map<string, Cell[]>>(
    new Map<string, Cell[]>()
  );
