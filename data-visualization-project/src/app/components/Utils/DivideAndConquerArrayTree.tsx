...
export class DivideAndConquerArray {
  ...
  constructor(array: number[], type: "mergesort" | "quicksort") {
    ...
    if (array.length > 0) {
      if (type === "mergesort") {
        ...
        this.createNodesOfArrayQuickSort(
          array,
          0,
          this.floorDivision(n - 1, 2)
        );
        this.nodes[0].value = array;
      }
    }
  ...
  getHeightForQuickSort(array: number[]): number {
    ...
    return (
      1 +
      Math.max(
        this.getHeightForQuickSort(leftArr),
        this.getHeightForQuickSort(rightArr)
      )
    );
  }
  ...
  createNodesOfArrayQuickSort(array: number[], i: number, j: number) {
    ...
    let nodeTree = new NodeTree(
      j,
      [],
      i * (this.distanceQuickSort + 50),
      j * (this.distanceQuickSort + 50)
    );
    this.Map.set(j, nodeTree);
    this.nodes.push(nodeTree);
    const pivot = array[array.length - 1];
  ...
  returnWidthForQuickSort() {
    return (
      (Math.pow(2, this.heightForQuickSort + 1) - 1) *
      (this.distanceQuickSort + 50)
    );
  }
